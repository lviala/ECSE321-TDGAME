...
public class ParticleTestState extends BasicGameState {
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		try {
			...
			Image image = new Image("res/particles/square_particle.png", false);
			system = new ParticleSystem(image,1500);
			
			File xmlFile = new File("data/test_emitter.xml");
			emitter = ParticleIO.loadEmitter(xmlFile);
			emitter.setPosition(400, 300, false);
...
float x = 300;
	...
	public void keyPressed(int key, char code){
		if(key == Input.KEY_SPACE){
			...
			try {
				Image image = new Image("data/square_particle.png", false);
				system = new ParticleSystem(image,1500);
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		g.drawString("It reloads the following files: \"data/test_emitter.xml\" and \"data/square_particle.png\"", 10, 55);
		system.render();
	}
