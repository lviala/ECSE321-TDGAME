...
public class RandomSelect_gui extends GUIState {
    ...
    public RandomSelect_gui(int id){
        super(id);
    }
    ...
    protected void buttonClicked(int buttonID, int mouseButton, int clickCount) {
        switch (buttonID){
            case BACK_ID:
                ...
                break;
            case LARGE_ID:
                MapGenerator.seletedLevel = null;
                MapGenerator.randomSize = MapGenerator.Size.LARGE;
                controller.getStateBasedGame().enterState(Core.PLAY);
                break;
            case MEDIUM_ID:
                MapGenerator.seletedLevel = null;
                MapGenerator.randomSize = MapGenerator.Size.MEDIUM;
                controller.getStateBasedGame().enterState(Core.PLAY);
                break;
            case SMALL_ID:
                MapGenerator.seletedLevel = null;
                MapGenerator.randomSize = MapGenerator.Size.SMALL;
                controller.getStateBasedGame().enterState(Core.PLAY);
                break;
        }
    }
